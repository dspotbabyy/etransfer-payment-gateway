...
class ETransfer_Payment_Gateway extends WC_Payment_Gateway {
    ...
    public function process_payment($order_id) {
        ...
        $api_endpoint = get_option('etransfer_backend_api_endpoint');

        if (!empty($api_endpoint)) {
            $order_data = array(
                'order_id'      => $order_id,
                'amount_cents'  => intval($order->get_total() * 100),
            ...
            if (is_wp_error($response)) {
                wc_add_notice(__('Payment processing error. Please try again.', 'etransfer-woocommerce'), 'error');
                return array(
                    'result'   => 'fail',
                    'redirect' => ''
                );
